...
- `delete_vendor_directories` is a boolean flag to indicate if the packages' vendor directories should be deleted after being processed. _default: true_. When enabled, Mozart still preserves the original vendor copy of a processed package if another installed package outside the Mozart-processed set depends on it.
