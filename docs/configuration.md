...
- **`delete_vendor_directories`** — whether to delete the packages' vendor directories after processing. _Default: `true`._ When enabled, Mozart still preserves the original vendor copy of a processed package if another installed package outside the Mozart-processed set depends on it.
