...
2. **Find the right place for it.** Read the existing docs in `docs/` and `CLAUDE.md` to determine where this lesson belongs. It should go in the file that someone would be reading when they encounter this situation. If it fits naturally into an existing section, add it there. Don't create a new file unless nothing existing fits.
...
5. **If this reveals a key gotcha** (something that would cause CI failures, subtle bugs, or wasted debugging time), consider adding a one-liner to the "Key things to know" section in `CLAUDE.md`.
