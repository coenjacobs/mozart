...
- **Zero configuration**: Mozart requires no `extra.mozart` block — running the command is the opt-in. All four core settings have sensible defaults. The inference chain is: `dep_directory` (static) → `classmap_directory` (mirrors `dep_directory`) → `dep_namespace` (from PSR-4 or package name) → `classmap_prefix` (derived from `dep_namespace`). Defaults are applied in `Mozart::applyDefaults()` before validation. See [docs/configuration.md](docs/configuration.md).
